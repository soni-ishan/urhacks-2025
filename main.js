...
canvas.addEventListener('wheel', (event) => {
    ...
    const zoomSensitivity = 0.0015; // tuning parameter
    const delta = -event.deltaY * zoomSensitivity;
    zoomLevel = Math.min(maxZoom, Math.max(minZoom, zoomLevel + delta));
    draw(); // redraw at new zoom
});
...
function drawNode(node, color) {
    ctx.beginPath();
    ctx.arc(node.x * zoomLevel, node.y * zoomLevel, NODE_RADIUS * zoomLevel, 0, 2 * Math.PI);
    ctx.fillStyle = color;
    ctx.fill();
    ctx.strokeStyle = '#1f2937';
    ctx.lineWidth = 2;
    ctx.stroke();
    ctx.moveTo(nodeFrom.x * zoomLevel, nodeFrom.y * zoomLevel);
    ctx.lineTo(nodeTo.x * zoomLevel, nodeTo.y * zoomLevel);
